...
| P1R-05 信号与退出 | VERIFIED | self-pipe 两端 O_NONBLOCK|FD_CLOEXEC（S-01 关闭）；handler async-signal-safe 无阻塞路径；进程 CTest（信号风暴 30× → 退出码 0 + DRAINED 恰一次幂等；TSan setarch + libcrypto suppression）；Debug/ASan/TSan 92/92；待提交 |
