...
| P3-06 接入 MessageAcceptance 协议与应用切片 | VERIFIED | 证据：ProtocolCodec adapter（msgid 11/12/13 与 errno 101..107 冻结，golden fixture 13 个字节级 pin）；ChatService 切片（在线直写/storeOffline 退役，B-11/B-12/B-17 迁移，B-18/B-19 收紧落地）；阶段 B 修复 codec 缺失字段 assert 中止（改 .at() → B-25 静默）；MessageAcceptanceApplicationTest 15 用例（chat_p306 独立库）+ ReliableProtocolGoldenTest 11 用例全绿，聚焦 28/28；DomainCharacterization 55 检查 MATRIX_ALL_PASS ×2；MultiReactorProcess 全绿；全量 Debug 273/273（构建目录 ~/muduo-chat-build/p3-06-debug）；进程 harness 自建 chat_p306（mysql+dbmigrate+--config）；本任务提交（feat(protocol): expose durable message acceptance，含 CMakeLists 注册），复审通过 |
