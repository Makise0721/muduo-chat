...
范围如实界定：claim 已批量（`claimBatchSize=100`），真正逐条 round-trip 在 processed 标记路径；P5-03A 的 HOL 同步语义冲突不适用（终态幂等标记）。详见 [P5-03C.md](../../archive/tasks/p5/P5-03C.md)。
...
详见 [P5-03D.md](../../archive/tasks/p5/P5-03D.md)。
...
- **移交行核对**：上节「移交 P5-04」成立——Buffer 分配复用候选由 P5-04 卡承接为「Buffer 内存池化」首批 spike（计划 §9.6），详见 [P5-04.md](../../archive/tasks/p5/P5-04.md)。
...
- **复评条件**：环境变化（WSL 内核启用 io_uring / 裸 Linux 部署），或后续 profile 显示分配 churn 回升为主要瓶颈。详见 [P5-04.md](../../archive/tasks/p5/P5-04.md)「阶段 1 复评与收口决定」。未改实现代码、未 stage。
